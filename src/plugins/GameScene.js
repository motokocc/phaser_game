...
export default class GameScene extends Phaser.Scene{
	...
	generateCharacterStats(charactersToPlay){
		charactersToPlay.forEach((character, index) => {
			this[`${character}_status_frame`] = this.add.sprite(this.padding/2, this.padding/2 + (index*120), 'char_status_frame')
			...
			this[`${character}_stats`] = this.player.playerInfo.cardsBattleData.filter(cardData => cardData.name == character)[0];
